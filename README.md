...
go build -o loan-ext -ldflags "-s -w"
- mac交叉编译：
...
关闭服务：
sudo systemctl stop loan-ext.service

启动服务：
sudo systemctl start loan-ext.service

设置开机自启：
sudo systemctl enable loan-ext.service

查看服务状态：
sudo systemctl status loan-ext.service

查看实时日志：
sudo journalctl -u loan-ext.service -f

指定时间范围过滤服务日志
journalctl -u loan-ext.service --since "2024-12-25" --until "2024-12-26"


go env -w GOOS=linux; go build -o loan-ext -ldflags "-s -w"
cd /usr/local/www/loan-ext/
sudo systemctl stop loan-ext.service
上传loan文件
chmod 755 loan-ext && sudo systemctl start loan-ext.service
sudo journalctl -u loan-ext.service -f
go env -w GOOS=windows

sudo systemctl restart loan-ext.service
```